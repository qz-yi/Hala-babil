  ...
  return (
    <Animated.View style={[{ transform: [{ scale: scaleAnim }] }]}>
      ...
      >
        ...
        />
        <View
          style={[styles.roomColorBar, { backgroundColor: accentColor }]}
        />
        ...
        <View style={styles.roomHeader}>
          ...
          >
            {ownerUser?.avatar ? (
              <Image
                source={{ uri: ownerUser.avatar }}
                style={styles.roomAvatarImg}
              />
            ) : (
              <Text style={styles.roomAvatarText}>
                {(ownerUser?.name?.[0] ?? room.name?.[0])?.toUpperCase() ||
                  "R"}
              </Text>
            )}
          </View>
          <View style={styles.roomInfo}>
            ...
            <Text
              style={[styles.roomOwner, { color: colors.textSecondary }]}
            >
              {room.ownerName}
            </Text>
          </View>
          ...
          <View style={styles.occupancyBadge}>
            <View
              style={[
                ...
                {
                  backgroundColor:
                    occupied > 0 ? "#10B981" : colors.textSecondary,
                },
              ]}
            />
            <Text
              style={[styles.occupancyText, { color: colors.textSecondary }]}
            >
              {occupied}/6
            </Text>
          </View>
  ...
  return (
    ...
    >
      ...
      <TextInput
        ...
        placeholder="ابحث عن مستخدم بالاسم أو الرقم..."
        placeholderTextColor={colors.textSecondary}
        textAlign="right"
...
function UserSearchResult({
  ...
  const accentColor = ACCENT_COLORS[user.id.length % ACCENT_COLORS.length];
  return (
    <TouchableOpacity
      style={[
        styles.searchResult,
    ...
    >
      ...
      >
        {user.avatar ? (
          <Image source={{ uri: user.avatar }} style={styles.searchAvatarImg} />
        ) : (
          <Text style={[styles.searchAvatarText, { color: accentColor }]}>
            {user.name[0]?.toUpperCase()}
          </Text>
        )}
      </View>
      <View style={{ flex: 1 }}>
        ...
        <Text style={[styles.searchPhone, { color: colors.textSecondary }]}>
          {user.phone}
        </Text>
      </View>
      <Ionicons
...
export default function HomeScreen() {
  ...
  const visibleRooms = rooms.filter(
    (r) => !r.isHidden || r.ownerId === currentUser?.id
  );
  const myRoom = currentUser
    ? rooms.find((r) => r.ownerId === currentUser.id)
    : null;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <LinearGradient
        colors={
          theme === "dark"
            ? ["rgba(79,70,229,0.2)", "transparent"]
          ...
          {isSuperAdmin && (
            <TouchableOpacity
              ...
              style={[
                styles.adminBtn,
                {
                  backgroundColor: `${colors.superAdmin}22`,
                  borderColor: colors.superAdmin,
                },
              ]}
            >
              <Text style={[styles.adminBtnText, { color: colors.superAdmin }]}>
                👑
              </Text>
            </TouchableOpacity>
          )}
      ...
      {isSearching ? (
        <FlatList
          ...
          renderItem={({ item }) => (
            <UserSearchResult user={item} colors={colors} />
          )}
        />
