...
- **Theme Engine v1 — multi-palette ThemePicker (v3.4)**: The old binary light/dark `Switch` in the profile drawer is replaced with a proper Theme Engine.
  - **Palette source**: `constants/colors.ts` now exports a `themes` record with 11 fully-typed palettes — `light`, `dark`, `neon`, `blueOcean`, `goldLuxury`, `purpleNight`, `redHacker`, `cinematic`, `glass`, `motion`, `creator`. Every palette satisfies the `ThemePalette` interface so any consumer doing `Colors[theme].card` continues to work after switching. Adding a 12th palette only requires adding an entry + listing it in `THEME_IDS`.
  - **Arabic display labels**: `THEME_LABELS_AR`, `THEME_DESCRIPTIONS_AR`, and `THEME_ICONS` (Feather names) live alongside the palettes so the picker UI stays in sync. Names are: فاتح، داكن، نيون، محيط أزرق، ذهبي فاخر، ليل بنفسجي، هاكر أحمر، سينمائي، زجاجي، حركي، مبدع.
  - **Context**: `AppContext` exports `Theme = ThemeId` (auto-derived from the palette record's keys) and a new `setThemeId(id)` action that persists to AsyncStorage under the same `"theme"` key as before. `toggleTheme()` is preserved for backward compatibility (cycles light↔dark only).
  - **`useColors()` hook**: rewritten to read from `AppContext.theme` instead of the device color scheme, so it stays in sync with the in-app picker. Falls back to `dark` if a stored id ever falls outside the current palette set.
  - **`components/ThemePicker.tsx`**: bottom-sheet `Modal` rendering a 2-column grid of theme cards. Each card shows three swatches sampled from the palette (`background`, `accent`, `text`), an icon bubble in the palette's accent color, the Arabic name, a one-line Arabic description, and a green check dot when active. Selection triggers `Haptics.selectionAsync()` on native, calls `setThemeId`, and closes the sheet. Cards are wrapped in `React.memo` to prevent re-render storms when the active theme changes.
  - **Profile drawer integration**: the dark/light Switch row in `app/(tabs)/profile.tsx` is replaced with a `TouchableOpacity` row labeled "المظهر" that shows the current palette's Arabic name on the right and opens the `ThemePicker` on tap. The picker is mounted inside the drawer's `Modal` so it stacks above the drawer correctly.
