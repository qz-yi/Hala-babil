...
- **Black-story-on-share root-cause fix via in-memory bridge (v3.5)**: The "share to my story" flow from feed posts (`(tabs)/index.tsx`), reels (`(tabs)/reels.tsx`), and the story viewer (`story/[userId].tsx`) used to hand the shared `mediaUrl` to the editor through Expo Router params (`router.push("/create-story", { sharedMediaUrl, ... })`). After the post-creation refactor that introduced the bake → finalize-post handoff, freshly-baked media URLs can be multi-MB `data:` URIs on web (or long `file://` paths on native), which Expo Router serializes into the navigation URL/state and silently truncates — the editor then received an empty/garbage `sharedMediaUrl`, rendered nothing in the canvas, and on publish saved a story with no media → black screen for everyone viewing it. Fix: introduced `lib/sharedContentBridge.ts` (mirrors `lib/bakedMediaBridge.ts`) — `setSharedContent(payload)` stashes the full payload in module-level memory and the source navigates with just `?sharedFromBridge=1`. `create-story.tsx` consumes the bridge once on mount via `consumeSharedContent()` and freezes the payload in `bridgePayload` state; `sharedPost` and `sharedMediaType` now derive from `bridgePayload` first, falling back to router params for backwards compatibility. `AppContext.shareContentToStory` and the StoryViewer share-to-my-story handler both stash through the bridge. The full URL is now preserved byte-for-byte regardless of length, so the share-to-story flow can no longer publish a black story even when the source media is a freshly-baked data URI.
- **Multi-modal editor with per-mode aspect ratios (v3.4)**: `app/create-story.tsx` (the UniversalEditor) now picks an `AspectMode` per `editorMode`: `story` = `fullscreen` (no constraint, fills the screen), `profile` = `square` (1:1 with a circular safe-area outline drawn on top of the canvas), `cover` = `wide` (16:9), `post` and `reel` = `natural` (uses the picked media's real width/height, falling back to 4:5 / 9:16 respectively). The aspect-locked surface is rendered as an inner `<View style={[st.aspectFrame, frameSizeStyle]}>` centered inside a `st.letterbox` band; `frameSizeStyle` is a memoized `{width, height}` clamped against the screen so very tall portrait media doesn't push the UI off screen. Natural aspect is captured by an `Image.getSize` effect on `mediaUri` (and another on `sharedPost.mediaUrl`); both write to a single `mediaNaturalAspect` state. A new `sharedMediaReady` flag is `false` until `Image.prefetch` resolves on the shared image — `handlePublish` early-returns with a toast while the flag is `false`, and a dim `sharedLoadingOverlay` veil with `ActivityIndicator` blocks taps over the canvas during that window. This is what fixes the "black snapshot of a still-loading sticker" failure mode for shared posts.
- **Reels seek removal + tap-to-fullscreen + black-story repost fixes (v3.3)**:
  1. *Reels UI cleanup*: Removed the `reelSeekRow` time/seek track block from `app/(tabs)/reels.tsx`. The reels feed no longer shows a progress/scrub bar — playback is purely tap-driven.
  2. *Home video — single tap to fullscreen, no separate button*: `PostVideoPlayer` in `app/(tabs)/index.tsx` no longer renders a floating fullscreen icon. Tapping the video itself calls `videoViewRef.current?.enterFullscreen()` (the imperative API on `expo-video`'s `VideoView`) and starts playback.
  3. *Black-screen-on-fullscreen fix*: The previous implementation mounted a SECOND `VideoView` inside a React Native `Modal` bound to the same `useVideoPlayer` instance. `expo-video` only allows one `VideoView` to actively present a player at a time, so the modal copy rendered black while audio kept playing. The Modal and its companion close button are deleted; the existing inline `VideoView` is what the OS expands into native fullscreen, so the surface is never duplicated and the picture is always in sync with the audio.
  4. *Exit-fullscreen freeze fix*: With the dual-mount removed, exiting fullscreen no longer has to tear down a second video surface, eliminating the post-exit freeze. `onFullscreenExit` also pauses the inline player so the feed doesn't keep audible playback running in the background.
  5. *Black-story bug on share* (image post / text post / mention story): `handlePublish` in `app/create-story.tsx` now follows a "FORWARD-ALWAYS RULE FOR RE-SHARES" — whenever a `sharedPost` is present we **never** route through `ViewShot.capture`. Image reposts forward `sharedPost.mediaUrl` directly. Text-only reposts (and re-shares of stories where the user was mentioned but had no media) publish with `mediaUrl=""`; the story viewer (`app/story/[userId].tsx`) already falls back to the accent `LinearGradient` plus the `sharedPost` sticker, so the original attribution is preserved instead of a black canvas. The hard guard at the bottom of `handlePublish` was extended with an `isSharedTextRepost` exception so an empty `finalUri` is allowed in that specific case. Root cause: `ViewShot` on Android captures `expo-linear-gradient` and not-yet-decoded remote `<Image>` content as solid black, which is exactly what the user saw on every shared-content story.
- **Black-story + missing-edits + nav-dup + laggy-drag fixes (v3.2)**:
  1. *Black story when sharing*: `handlePublish` in `create-story.tsx` now (a) takes a fast-path `canForwardSharedMedia` that forwards `sharedPost.mediaUrl` directly when the user added no edits, and (b) when an edit IS present, runs `Image.prefetch` + a 200ms warm-up before the `ViewShot.capture` so the remote image is fully decoded into the bitmap (previously the snapshot ran while `<Image>` was still loading → captured a black frame that was permanently saved as the story). On any capture failure it falls back to the original media URL.
  2. *Edits ignored on video stories*: `Story.overlays` in `AppContext` now carries the full `{x,y,scale,rotation,highlight,align}` metadata. Editor passes the complete overlay objects on publish, and `StoryViewer` reads `x/y/scale/rotation` and renders each overlay with a `transform: [translateX, translateY, scale, rotate]` in viewer-space (anchored at the canvas center, matching `DraggableText`'s coordinate system) instead of the old fixed `top: 34% + index*9%` stack. Legacy stories with no position fall back to the previous stacked layout.
  3. *Navigation duplicates*: All shared-content cards (`chat/[id].tsx`, `room/[id].tsx`) switched from `router.push` → `router.navigate` for posts/reels/stories so tapping a shared card unwinds the stack to the existing screen instead of mounting a new instance on top.
  4. *Laggy mini-room drag*: `FloatingRoomWidget` rewritten on top of `react-native-reanimated` worklets + `react-native-gesture-handler`'s `Gesture.Pan()`/`Gesture.Tap()` race. Position is `useSharedValue` (UI thread only); `onUpdate` writes translateX/translateY directly so dragging never crosses the JS bridge — true 60fps. On release, position is clamped inside the screen and persisted to `AppContext.floatingRoomPos` via `runOnJS`. Tap detection (open room) and pulse/glow loops also moved off the JS thread.
